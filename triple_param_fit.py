...
def find_theoretical_m_eff(redshift, *params): # params[0] Omega_Lambda, params[1] L*H_0**2, params[2] Omega_M
    def integrand(x):
        return 1 / (np.sqrt((params[2])*(1+x)**3 + params[0]))# + (1 - params[0] - (params[2]))*(1+x)**2))
    I = []
...
initials = np.array([0.70, 0.0017, 0.3])
...
print('Omega_M_0 = ({} \u00B1 {})'.format(popt[2], popt_err[2]))
...
plt.savefig('triple_fit', bbox_inches = 'tight')

plt.show()


print("All wrong, didn't correct for different S_k(r) value!!!")