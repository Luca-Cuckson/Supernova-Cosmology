...
def find_theoretical_m_eff(z, *params):
    Omegak = params[2]
    z_grid = np.linspace(0, np.max(z), 2000)
    integrand = 1 / (np.sqrt((params[1]) * (1+z_grid)**3 + params[2] * (1+z_grid)**2 + (1-params[1]-params[2])* (1+z_grid)**(3*(1+params[3]))))
    Integrals = integrate.cumulative_trapezoid(integrand, z_grid, initial=0)
    I_interp = scipy.interpolate.make_interp_spline(z_grid, Integrals, k=3)
    I_value = I_interp(z)
...
nsteps = 4000
p0 = np.array([-28.5, 0.3, 0.001, -1]) #chi-squared best-fit
nwalkers = 16
stepwidth = np.array([0.03, 0.06, 0.0003, 0.06]) #hopefully can figure this one out
burnin = 300
...
def get_values(chain):
    ...
    return np.mean(FunkyM_chain), np.mean(Omega_chain), np.mean(Omegak_chain), np.mean(w_chain), np.std(FunkyM_chain), np.std(Omega_chain), np.std(Omegak_chain), np.std(w_chain)
...
def walker_plot(all_walker_chains):
    ...
    for i in range(nwalkers):
        ...
        ax3.plot(path)
        ax3.set_ylabel("w")
    plt.savefig('Union2.5_walkerplot.svg', bbox_inches = 'tight')
...
FunkyM, Omega_Lambda, Omegak, w, FunkyM_err, Omega_Lambda_err, Omegak_err, w_err = get_values(chain)

labels = ["$M_B - 5log(H_0)$", "$\\Omega_{m}$", "$\\Omega_k$", "$w$"]
means = [FunkyM, Omega_Lambda, Omegak, w]
stds = [FunkyM_err, Omega_Lambda_err, Omegak_err, w_err]
...
def get_values2(chain):
    ...
    w_chain = chain[:,:,3]
    w_chain = w_chain[burnin:]
    return np.mean(FunkyM_chain), np.mean(Omega_chain), np.mean(Omegak_chain), np.mean(w_chain)
...
plt.savefig('Union2.5_cornerplot.svg', bbox_inches = 'tight')


#######################################################################################################################################
# Plotting Hubble Diagram

residuals = m_eff - find_theoretical_m_eff(z, FunkyM, Omega_Lambda, Omegak, w)
...
ys = find_theoretical_m_eff(smooth_x, FunkyM, Omega_Lambda, Omegak, w)
...
plt.savefig('Union2.5_zplot.svg', bbox_inches = 'tight')
