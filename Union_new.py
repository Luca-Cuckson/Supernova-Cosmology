...
nsteps = 3000
p0 = np.array([0.68, -28.5]) #chi-squared best-fit
nwalkers = 12
stepwidth = np.array([0.06, 0.03]) #hopefully can figure this one out
burnin = 300
...
def get_values(chain):
    ...
    return np.mean(Omega_chain), np.mean(FunkyM_chain), np.std(Omega_chain), np.std(FunkyM_chain)
...
def walker_plot(all_walker_chains):
    ...
    FunkyM_chain = all_walker_chains[:,:,1]
    ax0 = ax[0]
    ax0.set_ylim(0,1)
...
Omega_Lambda, FunkyM, Omega_Lambda_err, FunkyM_err = get_values(chain)

labels = ["$\\Omega_{\\Lambda}$", "$M_B - 5log(H_0)$"]
means = [Omega_Lambda, FunkyM]
stds = [Omega_Lambda_err, FunkyM_err]
...
def get_values2(chain):
    ...
    FunkyM_chain = chain[:,:,1]
    FunkyM_chain = FunkyM_chain[burnin:]
    return np.mean(Omega_chain), np.mean(FunkyM_chain)
...
residuals = m_eff - find_theoretical_m_eff(z, Omega_Lambda, FunkyM)
...
ys = find_theoretical_m_eff(smooth_x, Omega_Lambda, FunkyM)
